#!/usr/bin/env python3
"""
Standalone Enhanced Backtest with Win Rate Improvements
No database required - uses Binance API directly
"""
...
class StandaloneWinRateBacktest:
    ...
    def generate_enhanced_signals(self, df: pd.DataFrame, symbol: str) -> List[Dict]:
        ...
        print(f"      Sentiment: {sentiment_passed} ({sentiment_passed/total_candidates*100:.1f}%)")
        print(f"      Entry Timing: {entry_timing_passed} ({entry_timing_passed/total_candidates*100:.1f}%)")
...
def main():
    ...
    backtest = StandaloneWinRateBacktest()
